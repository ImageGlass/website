...
**Update 9.0.11.502**: The update addresses these [issues](https://github.com/d2phap/ImageGlass/releases/tag/9.0.11.502).
...
Updates:
- Since version 9.2: WebView2 Runtime is optional
- Since version 9.3: .NET Desktop Runtime 8 is not required.
...
- **"Start with OS to improve startup speed"** setting: ~~No alternative available~~ Requires version 9.2 or above.
...
- For power users:
  + Version 9 uses **Per-user setting** model which is different from **Per-machine setting** of version 8:
    ...
    - The app registry uses `HKEY_CURRENT_USER` for file type associations and app protocol. Since version 9.3, `HKEY_LOCAL_MACHINE` is used instead.
...
[Download ImageGlass 9.0](https://imageglass.org/release/imageglass-9-0-11-502-52){ .btn .btn-accent }
